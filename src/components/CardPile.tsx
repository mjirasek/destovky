...
const BISHOP_COLOR: Record<'bishop-light' | 'bishop-dark', string> = {
  'bishop-light': '#f0d9b5',
  'bishop-dark':  '#b58863',
};

export default function CardPile({ deck, color, isActive, canFlip, onFlipCard, layout }: Props) {
  ...
  if (layout === 'horizontal') {
    const cardW = 48, cardH = 66;
    const baseCard: React.CSSProperties = {
      width: `${cardW}px`, height: `${cardH}px`, borderRadius: '7px',
    ...
    return (
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
        ...
        <div style={{
          ...baseCard,
          border: `2px solid ${isActive && revealed ? '#629924' : '#3d3b38'}`,
          background: isActive && revealed ? '#1e2a0f' : '#262422',
          boxShadow: isActive && revealed ? '0 0 10px rgba(98,153,36,0.3)' : 'none',
        }}>
          {revealed ? (
            <>
              <span style={{ fontSize: '26px', lineHeight: 1, color: isWhite ? '#f0d9b5' : '#2a1a0e', textShadow: isWhite ? 'none' : '0 0 3px rgba(255,255,255,0.5)' }}>
                {SYMBOLS[revealed.type]}
              </span>
              {(revealed.type === 'bishop-light' || revealed.type === 'bishop-dark') && (
                <span style={{
                  width: '10px', height: '10px', borderRadius: '50%',
                  background: BISHOP_COLOR[revealed.type],
            ...
            <span style={{ fontSize: '9px', color: '#4a4744' }}>—</span>
          )}
        </div>
        ...
        <div
          style={{
            ...baseCard, position: 'relative', cursor: canFlip ? 'pointer' : 'default',
            border: `2px solid ${canFlip ? '#629924' : isActive ? '#4a6020' : '#3d3b38'}`,
            background: 'linear-gradient(135deg, #2c2926 0%, #1a1816 100%)',
            boxShadow: canFlip ? '0 0 12px rgba(98,153,36,0.45)' : 'none',
            transition: 'transform 0.15s, box-shadow 0.15s',
          }}
          onClick={canFlip ? onFlipCard : undefined}
          onMouseEnter={e => { if (canFlip) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(98,153,36,0.6)'; }}}
          onMouseLeave={e => { if (canFlip) { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 0 12px rgba(98,153,36,0.45)'; }}}
        >
          {remaining > 0 ? (
            <>
              {/* dot pattern */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 4px)', gap: '3px', opacity: 0.25 }}>
                {Array.from({ length: 9 }).map((_, i) => (
                  <div key={i} style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#a09070' }} />
                ))}
              </div>
              <span style={{ fontSize: '12px', fontWeight: 700, color: canFlip ? '#629924' : '#9e9b96', marginTop: '2px' }}>{remaining}</span>
              {canFlip && <span style={{ fontSize: '8px', color: '#629924', letterSpacing: '0.04em' }}>flip</span>}
            </>
          ) : (
            <span style={{ fontSize: '9px', color: '#4a4744' }}>—</span>
          )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isWhite ? 'column' : 'column-reverse', alignItems: 'center', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
        <span style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6e6b67', fontWeight: 600 }}>Drawn</span>
        {revealed ? (
          <div style={{
            ...
            {(revealed.type === 'bishop-light' || revealed.type === 'bishop-dark') && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                <span style={{
                  ...
                  background: BISHOP_COLOR[revealed.type],
                  border: `2px solid ${revealed.type === 'bishop-light' ? '#c8a870' : '#7a5a3a'}`,
                  flexShrink: 0, boxShadow: '0 1px 3px rgba(0,0,0,0.5)',
                }} />
      ...
      {/* Face-down deck */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
        <span style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6e6b67', fontWeight: 600 }}>Deck</span>
        <div
          ...
          {remaining > 0 ? (
            <>
              ...
              >
                ...
                <span style={{ fontSize: '13px', fontWeight: 700, color: canFlip ? '#629924' : '#4a4744', marginTop: '4px' }}>{remaining}</span>
